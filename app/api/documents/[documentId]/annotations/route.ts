...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (parsed.data.kind === "bookmark") {
    ...
    const bookmark = await prisma.bookmark.create({ data: { userId: session.user.id, documentId, sectionId: section.id, characterOffset: parsed.data.characterOffset, label: parsed.data.label } });
  ...
  if (parsed.data.kind === "highlight") {
    ...
    const highlight = await prisma.highlight.create({ data: { userId: session.user.id, documentId, sectionId: section.id, startOffset: parsed.data.startOffset, endOffset: parsed.data.endOffset, selectedText: parsed.data.selectedText, color: parsed.data.color } });
    return NextResponse.json({ kind: "highlight", annotation: highlight }, { status: 201 });
  }
  if (parsed.data.highlightId) {
    const highlight = await prisma.highlight.findFirst({ where: { id: parsed.data.highlightId, userId: session.user.id, documentId, sectionId: section.id }, select: { id: true } });
    if (!highlight) return NextResponse.json({ error: "Highlight not found." }, { status: 404 });
  }
  const note = await prisma.note.create({ data: { userId: session.user.id, documentId, sectionId: section.id, highlightId: parsed.data.highlightId, content: parsed.data.content } });
