...
export const createAnnotationSchema = z.discriminatedUnion("kind", [
  ...
  z.object({ kind: z.literal("highlight"), clientId: z.string().uuid().optional(), sectionId: z.string().min(1), startOffset: z.number().int().nonnegative(), endOffset: z.number().int().positive(), selectedText: z.string().trim().min(1).max(10_000), color: z.enum(["yellow", "green", "blue", "pink"]).default("yellow") }),
  z.object({ kind: z.literal("note"), clientId: z.string().uuid().optional(), sectionId: z.string().min(1), highlightId: z.string().min(1).nullable().optional(), content: z.string().trim().min(1).max(20_000) }),
]);
